...
```{python, echo=FALSE}
import analyze
_, _, _ = analyze.analyze(last=1000)
```
...
Gdy wziąć taki sam kapitał i jako zakres dać cały okres spółki Makarony Polskie na giełdzie, to mielibyśmy
`r prettyNum(round(py$capital,2), big.mark = " ", decimal.mark = ",", scientific = FALSE)`
zł, jest to spadek prawie 80%. Tymczasem, gdybyśmy po prostu kupili na samym początku i sprzedali na samym końcu tego okresu,
wtedy mielibyśmy
`r prettyNum(round(py$capital_do_nothing,2), big.mark = " ", decimal.mark = ",", scientific = FALSE)`
zł.
